...
import { initFeedCards, type VideoMeta } from './feedCards';
import {
...
function syncWatchPanelLabels(): void {
  syncWatchPanelLabelsUi({
    ...
    inLibrary,
    panelT,
    onAfter: () => jpWatchPanelDebugStrip.sync(),
  });
}
...
/** When `chrome.storage.onChanged` delivers a full `PersistedData`, merge practice totals before async `GET_STATE` (covers tabs with no watch `videoId`, where `refreshState` skips library fetch). */
...
function showLibraryBanner(text: string, tone: 'ok' | 'err' | 'warn'): void {
  showWatchPanelLibraryBanner({ shadowRoot, text, tone, debug: panelMountDebug });
}

function needsHomeFeedAttention(): boolean {
  return needsHomeFeedPanelAttention(getVideoIdFromUrl);
}
...
function boot(): void {
  ...
  attachYoutubeNavHooks(() => fireAsyncWatch(onVideoChanged()));
  attachYoutubePlayerDomHooks(() => fireAsyncWatch(onVideoChanged()));
  attachHomeFeedPointerPick({
    elementInOurUiShell,
  ...
  attachPracticePageFlushListeners({
    getPauseWhenUnfocused: () => settingsCache.pauseWhenUnfocused,
    flush: flushPractice,
  });
